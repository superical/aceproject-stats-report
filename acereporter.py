...
argparser.add_argument('--subject', nargs='?', default='Project Overview', type=str)

argparser.add_argument('--BURNDOWN_LAST_NUM_DAYS', nargs='?', const=7, default=7, type=int)
argparser.add_argument('--burndown-exclude-today', dest='burndown_exclude_today', action='store_true')
argparser.set_defaults(burndown_exclude_today=False)
...
argparser.add_argument('--detailed-summary', dest='detailed_summary', action='store_true')
argparser.set_defaults(detailed_summary=False)
...
BURNDOWN_EXCLUDE_TODAY = args.burndown_exclude_today
...
USE_DETAILED_SUMMARY = args.detailed_summary
...
if BURNDOWN_EXCLUDE_TODAY is True:
    burndown_endDate = burndown_endDate - datetime.timedelta(days=1)
...
tasksPendingUat = getTasksPendingUatData(TASKS_PENDING_UAT_MIN_DAYS)
...
emailData = {
    ...
    'useDetailedSummary': USE_DETAILED_SUMMARY,
    'currentDateTime': datetime.datetime.now(),
    'execTimeTakenSeconds': round(time.time() - prog_start_time, 3),
...
subject = "[{0}] {1}".format(project.projectName, EMAIL_SUBJECT)
